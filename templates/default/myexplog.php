...
<div class="user_list">
  ...
  <tr class="order_list_title">
    <td width="190" bgcolor="#F1F1F1">积分日期</td>
    <td width="140" bgcolor="#F1F1F1">类型</td>
    <td bgcolor="#F1F1F1">来源</td>
    <td bgcolor="#F1F1F1">积分</td>
    
  </tr>
  <?php foreach($orderlist as $list){?>
  <tr class="list_order">
    <td bgcolor="#DBF3FF"><?php echo date('Y-m-d H:i:s',$list['LogTime']);?> </td>
    <td bgcolor="#DBF3FF"><?php echo $this->model_config->explog_type_ary($list['IncomePayout']) ?></td>
    <td bgcolor="#DBF3FF"><?php  echo $list['soure'].':'.$list['hotel_name'].'<br>'.$list['orderNum'] ?></td>
    <td bgcolor="#DBF3FF"><?php echo $list['ExpNum'] ?></td>
  </tr>
   <?php }?>
   <tr>
