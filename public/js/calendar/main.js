...
(function($,options){ $.fn.loading = function(options){
	...
	return this.each(function() {
		...
		if(o.div=="body"){
			...
			var r=$("<div class='pop_load loading'>").css({left:($(window).width()/2)-150,top:$(window).height()/2-100}).html("<div class='pop_loading'><dl><dt>请稍等，您查询的酒店正在搜索中......</dt><dd>预订酒店完全免费，全国20000多家酒店任您选！</dd></dl></div>");
		}else{
			q=$(""+o.div+"");
			...
			if(o.mini){
					...
					var r=$("<div class='pop_load loading'>").css({position:"absolute",left:($(window).width()/2)-150,top:$(window).height()/2-100}).html("<div class='pop_loading'><dl><dt>请稍等，您查询的酒店正在搜索中......</dt><dd>预订酒店完全免费，全国20000多家酒店任您选！</dd></dl></div>");
				}
		}
...
function checkindex(){
	...
	if(!f.minprice.value)f.minprice.value='';
	if(!f.maxprice.value)f.maxprice.value='';
	if(isNaN(f.minprice.value)||isNaN(f.maxprice.value))
	{
		alert("房价范围必须为数字!");
		f.minprice.focus();return false;
	}
	if((f.minprice.value*1>=f.maxprice.value*1)&&f.maxprice.value!='')
	{
...
function refreshprice(id,Order_api){
	...
	if(tm1!=''&&tm2!=''){
	$("#h"+id+"").html("<p style='text-align:center;'><img src='"+webpath+"js/loadprice.gif' /></p>");
	if(id.indexOf("A")>-1){
		
		file = '/hotelinfo/getroom?hid='+id+'&tm1='+tm1+'&tm2='+tm2;
	}
	else{
...
function dobook(a,b,c){
	if(document.doBook.loginstate.value==1){
	...
	document.doBook.submit();
		...
		var url='/ebook/index/?hid='+a+'&rid='+b+'&pid='+c+'&tm1='+document.doBook.tm1.value+'&tm2='+document.doBook.tm2.value+'';
		url=encodeURIComponent(url);
		Dialog.frame('login','/user/ajaxlogin/?url='+url,'快速登录','610','290',1);
	}
}
function rotatorimg(i,imgwith,imgheight){
	if(imgwith==0 || imgheight=="")
		imgwith = 400;
	if(imgheight==0 || imgheight=="")
		imgheight = 210;
	$("#Slider").stop(true,false).animate({top : -imgheight*i},imgwith);
