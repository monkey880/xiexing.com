...
class Comment extends CI_Controller 
{
	...
	{
        //载入布局模型
        $layout = $this->model_layout->get_layout('comment');
        $data = array();
        ...
        $perPage = 10 ;
        $listresult = $this->model_hotel->getCommentListByCiytid($cityid,10,$perPage,$nowPage);
        $list = $listresult['reqdata'];
        $allnums = $listresult['retHeader']['totalput'];
        //生成页码
        $config['base_url'] = base_url()."/comment/$cityid";
        ...
        $data['allnums'] = $allnums ;
         ...
         //处理网页中的seo信息
        $keywords_array = $this->model_common->getKeywords('comment');
        $keywords_search = array('{doname}','{classname}','{cityname}');
        $keywords_replace = array(CFG_WEBNAME,'酒店点评信息',$cityname);
