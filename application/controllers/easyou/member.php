...
class Member extends CI_Controller 
{
    ...
    {  
        ...
        $managerCount = $this->model_member->getMemberCount();
        $pageinfo = $this->tool->get_page_info($page,$managerCount,$pagesize);   
        $managerList = $this->model_member->get_user_list($pageinfo['start'],$pagesize);
		
        //分页
        $config['base_url'] = base_url(CFG_ADMINURL.'/member/index/');
        ...
        $this->load->view('admin/admin_memberlist',$data);
    }
    ...
    {    
    	...
    	if ($id > 0 ) {
    		...
    		$info = $this->model_member->get_userinfo($id,2);
        } else {
        ...
        $data = $this->userinfo;
        $data['info'] = $info;
        $this->load->view('admin/admin_member_add',$data);
    }
	  ...
	  public function del_member ()
    {
        ...
        $managerid = rtrim($manageridStr,',');
    	if (!empty($managerid)) {
    		$result = $this->model_member->delmember($managerid);
    		if ($result) {
    			redirect(CFG_ADMINURL.'/member');
    		} else {
    			redirect(CFG_ADMINURL.'/member');
    		}
    	}
