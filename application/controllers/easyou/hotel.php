...

{
    ...
    {    
        ...
        if ($cityid) {
            ...
            $whereSql = array('CityID'=>$cityid); 

            $whereSqlList = "where CityID=$cityid";   
		...
		if($keywords){
			  ...
			  $whereSql = array('HotelName'=>$keywords); 

            $whereSqlList .= " AND HotelName LIKE '%$keywords%'";   

			}

		

        $hotelcount = $this->model_hotel->get_count($whereSql);

        $pageinfo = $this->tool->get_page_info($page,$hotelcount,$pagesize);

        $hotellist = $this->model_hotel->get_list($pageinfo['start'],$pagesize,'hid desc',$whereSqlList);
        ...
        $this->load->view('admin/admin_hotelslist',$data);
    ...
    {    
		...
		foreach($list as $hotel){
			...
			$data['eareaid'] = $hotel['eareaid'];
			
			$data['eareaname'] = $hotel['eareaname'];
			...
			$data['cbd_name'] = $hotel['esdname'];
			
			$data['cbd_id'] = $hotel['esdid'];
    ...
    {    
		...
		$hotelinfo=$this->model_hotel->getHotelInfo($orderinfo['hotel_id']);
		$userinfo=$this->model_member->get_userinfo($orderinfo['user_id'],2);
		$bltime=explode('-',$orderinfo['rkTime']);
		
		//已入住
		if($state==1){
			$smscon="携行网确认：您预订的".date('m月d日',$orderinfo['CheckInDate'])."入住 ".date('m月d日',$orderinfo['CheckOutDate'])."离店".$orderinfo['hotel_name'].$orderinfo['roomtitle'].$orderinfo['roomnumber']."间".$orderinfo['tian_num']."晚总价".$orderinfo['roomallprice']."元，已预订成功，酒店地址：".$hotelinfo['Address']."。保留到".$bltime[1]."点，如不能准时到店或行程有变请联系021-24208963或24小时热线4006002069,www.xexing.com ";
		
		$res=$this->sendsms->get_message($orderinfo['phone'],iconv('utf-8','gb2312',$smscon));
		
		//$res2=$this->sendemail->send($orderinfo['email'],"您已成功预定【".$z['hotelname']."】",$smscon);
		//print_r($orderinfo['email']);
		}
		else if($state==2&&$orderinfo['user_id']){
		...
		else if($state==4&&$orderinfo['user_id']){
	...
	public function tocsv()
	{
      ...
      public function baidu_lbs_list()
    ...
    {    
       ...
       $api_list=file_get_contents("http://api.map.baidu.com/geodata/v2/poi/list?geotable_id=34599&ak=85654a7702d8b2163b85f87e6585b4f5&page_size=$pagesize&page_index=$page");
		$api_list=json_decode($api_list);

		

        $hotelcount = $api_list->total;

        $pageinfo = $this->tool->get_page_info($page,$hotelcount,$pagesize);

        $hotellist = $api_list->pois;
        ...
        $config['base_url'] = base_url(CFG_ADMINURL.'/hotel/baidu_lbs_list/'.$cityid);
        ...
        $this->load->view('admin/admin_hotelslist_lbs',$data);

    }   
	
	
	 public function tongbu_lbs()
    ...
    {    
		...
		foreach($list as $hotel){
			$oapi_list=file_get_contents("http://api.map.baidu.com/geodata/v2/poi/list?geotable_id=34599&ak=85654a7702d8b2163b85f87e6585b4f5&api_id=$hotel[ID]");
		$api_list=json_decode($api_list);
			...
			$data['longitude'] = $hotel['baidu_lng'];
			
			$data['latitude'] = $hotel['baidu_lat'];
			
			$data['eareaid'] = $hotel['eareaid'];
			
			$data['coord_type'] = '3';
			
			$data['api_id'] = $hotel['ID'];
			
			$data['picture'] = $hotel['Picture'];
			
			$data['liansuo'] = $hotel['liansuo'];
			
			$data['xingji'] = $hotel['xingji'];
			
			$data['min_jiage'] = $hotel['min_jiage'];
			
			if(stripos($hotel['Service'],"停车")){
			$data['tingchechang'] = 1;
			}
			if(stripos($hotel['Service'],"wifi")){
			$data['wifi'] = 1;
			}
			if(stripos($hotel['Service'],"早餐")){
			$data['zaocan'] = 1;
			}
			
			if(stripos($hotel['Service'],"商务")){
			$data['shangwu'] = 1;
			}
			
			$data['websiteid'] = 1;
	
			$data['ecityid'] = $hotel['ecityid'];
	
			$data['haoping'] = strtok($hotel['df_haoping'],'$');
			
			$data['esdid'] = $hotel['esdid'];
			
			$data['esdname'] = $hotel['esdname'];
			...
			if ( $oapi_list->total>0 ) {
	
			   
				$result = $this->model_hotel->post_baidu($oapi_list->pois[0]->id,$data,'update');
	
				if ($result) {
	
					$res.= $data['title'].',修改酒店成功<br>';
	
					
	
				} else {
	
					$res.=  $data['title'].',修改酒店失败<br>';
				...
				$result = $this->model_hotel->post_baidu('',$data,'create');
				
				
				if ($result) {
					
					$res.=  $data['title'].',添加酒店成功<br>';
		...
		if($page<$totalpg){
		$page++;
		$data['url']="/easyou/hotel/tongbu_lbs/?index=$index&page=$page";
		}
		else{
			if($index<886){
			$index++;
		$data['url']="/easyou/hotel/tongbu_lbs/?index=$index&page=1";
			}
			//$data['url']="/easyou/hotel";
		}
        $this->load->view('admin/admin_hotel_tongbu',$data);

    }
	
	 public function del_baidu()

    {
    	...
    	if(!empty($newsid)){
    		...
    		$result = $this->model_hotel->post_baidu($newsid,'','delete');

    		if($result){

    			redirect(CFG_ADMINURL.'/hotel/baidu_lbs_list/'.$urlClassid);
