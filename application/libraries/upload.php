...
class Upload {
	...
	function hpicshow($hotelpictures,$limit=5)
	{
	    ...
	    foreach ($pic_ary AS $k => $v) {
	        ...
	        $result[$k]['pic_thumb_url'] = $pic_all_array[0];
	        $result[$k]['pic_url'] = $pic_all_array[1];
	...
	*/
    ...
    function change_array($questionList)
    {
   		foreach ($questionList as $key=>$val) {
        	$questionList[$key] = str_replace(array('住哪网','http://www.zhuna.cn/','www.zhuna.cn','http://www.zhuna.cn','hotel-'),array(CFG_WEBNAME,CFG_INDEXURL,CFG_INDEXURL,CFG_INDEXURL,'hotelinfo/'),$val);
